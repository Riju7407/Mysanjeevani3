...
export default function DoctorWallet() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      const id = user.doctorId || user.id;
      setDoctorId(id);
    }
  }, []);
  ...
  useEffect(() => {
    if (!doctorId) return;
    fetchTransactions();
  }, [doctorId, selectedTimeframe]);

  const fetchWalletData = async () => {
    try {
      const res = await fetch(`/api/wallet/balance?doctorId=${doctorId}`);
      const data = await res.json();
      if (data.wallet) {
        setWallet(data.wallet);
      }
      
      const bankRes = await fetch(`/api/bank-details?doctorId=${doctorId}`);
  ...
  const fetchEarnings = async () => {
    try {
      const res = await fetch(`/api/wallet/earnings?doctorId=${doctorId}`);
      const data = await res.json();
      if (data.summary) {
  ...
  const fetchTransactions = async () => {
    try {
      const res = await fetch(`/api/wallet/transactions?doctorId=${doctorId}&timeframe=${selectedTimeframe}`);
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-100 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/doctor/panel" className="text-indigo-600 hover:text-indigo-800 mb-4 inline-block">
            ← Back to Dashboard
          </Link>
              ...
              <div>
                ...
                {bankDetails ? (
                  <div className="bg-gray-50 rounded-lg p-6">
                    ...
                    <Link
                      href="/doctor/wallet/edit-bank-details"
                      className="mt-6 inline-block bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-6 rounded-lg transition"
                    >
                      Edit Bank Details
                    </Link>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 text-center">
                    ...
                    <Link
                      href="/doctor/wallet/add-bank-details"
